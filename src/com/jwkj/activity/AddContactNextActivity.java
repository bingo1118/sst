...
public class AddContactNextActivity extends BaseActivity implements
		...
		public void onReceive(Context arg0, Intent intent) {
			if(intent.getAction().equals(Constants.P2P.RET_GET_BIND_ALARM_ID)){
				...
				if (count == 0) {
					...
					P2PHandler.getInstance().setBindAlarmId(mSaveContact.contactId,
							mSaveContact.contactPassword, new_data.length, new_data);
				}
			}
	...
	private void saveCamera(){
		...
		map.put("cameraID", mSaveContact.contactId);
		map.put("userID", NpcCommon.mThreeNum);
		map.put("cameraPassword", input_pwd);
		map.put("cameraName", str);
		map.put("cameraType", mSaveContact.contactType+"");
		map.put("messageCount", mSaveContact.messageCount+"");
		map.put("phoneUUID", NpcCommon.mThreeNum);
				...
				new Listener<JSONArray>() {
					...
					public void onResponse(JSONArray response) {
						if(response.length()>0){
							try {
								if(response.getString(0)!="success"){
						...
						isSave = true;
						sendSuccessBroadcast();
						P2PHandler.getInstance().getBindAlarmId(mSaveContact.contactId,mSaveContact.contactPassword);
						System.out.println("AddContactNextActivity..2");
						finish();
					}
				}, 
				new ErrorListener() {
	...
	class MyTask extends AsyncTask<String, Integer, Object>{
		...
		protected Object doInBackground(String... params) {
			String defenceID = NpcCommon.mThreeNum+mSaveContact.contactId;
