...
public class MainControlActivity extends BaseActivity implements OnClickListener{
	...
	protected void onCreate(Bundle arg0) {
		...
		replaceFragment(FRAG_MAIN,false,true,null);
	}
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver(){
		...
		public void onReceive(Context arg0, Intent intent) {
			...
			if(intent.getAction().equals(Constants.P2P.RET_GET_DEFENCE_AREA)){
				...
				List<String> listStr = initData(data,contact.contactId);
				replaceFragment(FRAG_DEFENCE_AREA_CONTROL,true,true,listStr);
			}
	...
	public void replaceFragment(int type,boolean isAnim,boolean isEnforce,List<String> liStr){
		...
		try{
			...
			if(FRAG_DEFENCE_AREA_CONTROL==type){
				getDefenceAlarmType(liStr,fragment,type);
			}else{
				current_frag = type;
	...
	private void getDefenceAlarmType(List<String> defenceList,final Fragment fragment,final int type){
		if(null!=defenceList&&defenceList.size()>0){
			RequestQueue mQueue = Volley.newRequestQueue(mContext);
			Map<String,String> map = new HashMap<String,String>();
			map.put("defenceSize", defenceList.size()+"");
			for(int i=0;i<defenceList.size();i++){
				map.put("defenceID["+i+"]", defenceList.get(i));
			}

			JsonArrayPostRequest mJsonRequest = new JsonArrayPostRequest(
					Constants.GET_ALARM_TYPE_LIST_URL, 
					new Listener<JSONArray>() {
						@Override
						public void onResponse(JSONArray response) {
							if(response.length()>0){
								List<Defence> mList = new ArrayList<Defence>();
								String str = contact.activeUser;
								for(int i=0;i<response.length();i++){
									try {
										Defence mDefence = new Defence();
										JSONObject obj = (JSONObject) response.get(i);
										mDefence.mDefenceID = str+obj.getString("cameraDefenceID");
								...
								DataManager.updateAlarmType(mContext,mList);
							}
							current_frag = type;
							transaction.replace(R.id.fragContainer, fragment, fragTags[current_frag]);
	...
	public List<String> initData(ArrayList<int[]> data,String cameraID){
		...
		for(int i=0;i<data.size();i++){
			...
			for(int j=0;j<status.length;j++){
				...
				if(status[j]==1){
					...
					String defenceIDStr = cameraID+"0"+i+"00"+j+"";
					li.add(defenceIDStr);
				}
			}
		}
