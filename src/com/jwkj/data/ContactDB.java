...
public class ContactDB {
	...
	public Contact findByContactId(String cameraId) {
		//List<Contact> lists = new ArrayList<Contact>();
		Contact data = null;
		Cursor cursor = null; 
		cursor = myDatabase.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE "
				+ COLUMN_CONTACT_ID + "=?" + " order by " + COLUMN_CONTACT_ID,
				new String[] { cameraId });
		if (cursor != null) {
			data = new Contact();
			while (cursor.moveToNext()) {
	...
	public boolean ifExitByActiveUserIdAndContactId(String activeUserId,
			String ContactId) {
		Cursor cursor = null;
		cursor = myDatabase.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE "
				+ COLUMN_ACTIVE_USER + "=? AND " + COLUMN_CONTACT_ID + "=?",
